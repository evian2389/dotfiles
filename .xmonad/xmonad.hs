...
myLayoutHook = showWorkspaceName
             ...
             $ mirrorToggle
             $ reflectToggle
             $ flex ||| tabs ||| threeCol ||| bsp ||| oneBig ||| simpleTall ||| simpleThree
  where
...
myKeys conf = let
    ...
    , ("S-C-q"                  , addName "Rebuild & restart XMonad"        $ confirmPrompt hotPromptTheme "Quit XMonad" $ io (exitWith ExitSuccess))
  ...
  --, ("M-7"                    , addName "TESTING"                         $ runInTerm "-name glances" "glances" )
    ...
    , ("M1-p"                   , addName "Capture screen"                  $ spawn "flameshot gui" )
    , ("M1-S-p"                 , addName "Capture screen - area select"    $ spawn "flameshot" )
